...
重做日志（redo log）、[回滚](https://so.csdn.net/so/search?q=回滚&spm=1001.2101.3001.7020)日志（undo log）、二进制日志（binlog）、错误日志（errorlog）、慢查询日志（slow query log）、一般查询日志（general log），中继日志（relay log）。

redo log属于MySQL存储引擎**InnoDB**的事务日志。

### 3 MySQL的逻辑架构

![img](assets/1680785668335-fd8527b7-44ab-4318-b9f2-c02a475da28a.png)
...
![img](assets/1680785834250-5104bc90-47ae-463b-b060-a2d972c2df34.png)
...
![img](assets/1599812260233-ab712bb5-89a1-4b48-8232-cac49b5a0d28.png)
...
![img](assets/1599812307393-b9378fca-a484-4985-afd8-c846cfd0b8ac.png)
...
![img](assets/1599812358362-5cd6f729-2d85-4d21-ab6b-e79980ebb802.png)

![img](https://cdn.nlark.com/yuque/0/2020/png/2517043/1599812372720-e9d957a1-fba5-4464-9c73-4e79d00abb80.png)
...
![img](https://cdn.nlark.com/yuque/0/2020/png/2517043/1599812416723-003f94ee-236a-4398-b169-0e5287089043.png)
...
InnoDB引擎下默开启索引下推，是sql优化的机制之一

举例来说 学生表 有id name class age 字段，我需要查询5班年龄大于10岁的学生

```java
SELECT * FROM STU WHERE class=5 and age>10
```

没有索引下推时存储引擎分别比较calss=5的数据满足一个条件就把符合条件的数据返回到server层，由server层进行数据的比对age>10满足后进行返回

索引下推的意思是，将过滤条件下推到存储引擎层进行过滤，也就说执行引擎在查询过程中会直接判断calss=5 AND age>10的两个条件，满足两个条件的数据才返回到server层，避免大量数据读取到内存中减少io操作，通过执行引擎层根据where条件对数据进行过滤
...
![img](assets/1599812491743-a8a88a72-b227-4a53-bbb0-1bad9ef04286.png)
...
![img](assets/1599812517065-0fcebe5a-0c71-4a3b-94f0-ebfb3febd4f9.png)
...
![img](https://cdn.nlark.com/yuque/0/2020/png/2517043/1599812727867-7b926226-550f-4ff6-a013-37cbc0fa1aa9.png)
...
![img](assets/1599812882308-c4ebe2f1-aab6-4ca4-a71a-b15630b12730.png)
...
### 23 B+树在满足聚簇索引和覆盖索引的时候不需要回表查询数据，
...
![img](assets/1599812933861-cfaa2b6a-6054-412d-bd40-6e08ae27693b.png)

### 25 回表查询是什么意思？

回表查询（Ref、Index Ref、Index Scan）是指在使用索引进行查询时，当需要访问表中未包含在索引中的其他列时，MySQL需要根据索引中的键值回到表中检索相应的行数据。这个过程就称为回表查询。



举个例子，假设我们有一个表`students`，包含`id`、`name`和`age`字段，并且我们在`age`字段上创建了一个索引。如果我们执行以下查询：
...
在这个查询中，虽然我们在`age`字段上有一个索引，MySQL可以使用这个索引快速地定位到满足条件的行，但是由于我们还需要返回`id`和`name`字段的值，这些字段并没有包含在`age`索引中，因此MySQL需要回到表中检索这些额外的列数据。这个过程就是回表查询。



回表查询会增加额外的IO操作和内存消耗，尤其是在返回大量数据或者有大量查询的情况下。因此，在设计索引时，需要根据实际查询需求考虑哪些列需要包含在索引中，以尽量减少回表查询的次数，提高查询性能。

### 26 聚簇索引和非聚簇索引都是什么意思？

在MySQL中，聚簇索引（Clustered Index）和非聚簇索引（Non-clustered Index）是指索引在物理存储上的组织方式，它们的区别主要在于索引和数据行的存储关系。
...
`select age from employee where ``age < 20`的查询时，在索引的叶子节点上，已经包含了age信息，不会再次进行回表查询。

### 28 联合索引是什么？为什么需要注意联合索引中的顺序？
...
![img](assets/1599812960554-0af20f22-4c0f-4de9-82f2-615f5a12a68a.png)
...
![img](assets/1599813010754-f03f7267-050c-4739-8575-3adbb16b993f.png)
...
![img](https://cdn.nlark.com/yuque/0/2020/png/2517043/1599813061558-c89e951f-7069-452c-9f18-84eea994be80.png)
...
![img](assets/1599813252597-4d4a4dd3-2208-4126-9a53-3228128b530f.png)

S表

![img](assets/1599813279642-c5a4ec74-30f1-4222-a469-c8d6dc71ed24.png)
...
![img](assets/1599813324487-c8e26c5a-b85b-49b5-9755-56cac8801fae.png)
...
![img](assets/1599813372348-d82da8b9-6e9d-4a22-9fd9-85e3c93400bb.png)
...
![img](assets/1599813419791-8603215a-aa86-4d7b-b65e-a537a062eb12.png)
...
![img](assets/1599813462311-894cf53d-0951-4cc6-b45e-5b03ed53f35c.png)
...
![img](assets/1599813502878-47a6bf45-5efb-44e2-afa6-bdc7ef2f5797.png)
...
![img](assets/1599813643329-87980e24-8ae6-4864-89d5-e14f3a1f7800.png)
...
![img](assets/1599813669937-39f0f943-1c13-4a31-ac24-4ed6f8c1a693.png)
...
![img](assets/1599813697636-542c3092-6292-481d-8e8e-20208d84183f.png)
...
![img](assets/1599813817748-76cba05a-750d-462a-9815-802cf6f854a5.png)
...
- `select * from table where age > 20 limit 1000000,10`这种查询其实也是有可以优化的余地的. 这条语句需要load1000000数据然后基本上全部丢弃,只取10条当然比较慢. 当时我们可以修改    
...
![img](assets/1599814010166-d7c067e1-d66f-4515-b194-ee4062964e57.png)

开启慢查询日志

配置项：`slow_query_log`

可以使用`show variables like ‘slov_query_log’`查看是否开启，如果状态值为`OFF`，可以使用 

`set GLOBAL slow_query_log = on`来开启，它会在`datadir`下产生一个`xxx-slow.log`的文件。
...
![img](assets/1599814270500-ac3fab7d-a992-4687-9c5d-8bfc787cf1e4.png)
...
![img](assets/1599814293905-eb8f7569-b383-41c1-b4ef-f89ca565653a.png)
...
![img](assets/1599814320089-dbd3abf9-c6fe-4568-bda8-a3760da97f71.png)
...
![img](assets/1599814354124-9478fa07-de18-4da9-aabd-70cba86a5394.png)
...
![img](assets/1599814391747-d331eb22-aca5-4ad9-a953-aeb0c47b5e81.png)
...
![img](assets/1599814419275-736212dd-86a8-4e87-a8c6-46a4bd24786a.png)
